...
public class LogController  extends BaseController{
    private static final Logger logger= org.apache.log4j.Logger.getLogger(LogController.class);
    @Autowired
    ...
    public String showLog(SysLog sysLog, String page, String limit){
        Page<SysLog> sysLogs = PageHelper.startPage(Integer.valueOf(page),Integer.valueOf(limit));
        List<SysLog> log = null;
        try{
            log = logMapper.selectListByPage(sysLog);
        }catch (MyException e){
            logger.error(e.getMessage());
        }
        ReType reType = new ReType(sysLogs.getTotal(),log);
